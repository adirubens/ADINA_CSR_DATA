...
-- ===============================================================================================================================
-- FUTURE WORK: 
-- 1.- Create a User input table for the CSR Customer List
-- 1.1.- Add customer_id, ship_to_id from the initial queries that generates raw_data. 
-- 1.2.- Filter Company 413 (CAN) for P21 Dataset
-- 2.- Includes Harmonize tables for credit
-- 3.- Consolidate all in an stored procedure
-- 4.- Create a DBT job to run on daily basis
...
SET REPORTROWS = 50;
  -- set dates
	...
	SELECT	DT.DATE_ID, DT.FY, DT.FM, DT.YYYY, DT.MM
			, CUST.CUSTOMER_ID
			, CUST.SHIP_TO_ID -->> Hold SHIP_TO_ID instead of LEGACY_CUSTOMER for p21
	...
	GROUP BY DT.DATE_ID, DT.FY, DT.FM, DT.YYYY, DT.MM
			, CUST.CUSTOMER_ID, CUST.SHIP_TO_ID
    ...
    AS   
    (
		SELECT	CUSTOMER_ID, SHIP_TO_ID, LEGACY_CUSTOMER
		FROM	DIM_CUSTOMER AS CM 
		WHERE	EXISTS ( SELECT * FROM DB_ADI_NA_QA.SCH_ADI_NA_CORE.T_RAWDATA AS FILT WHERE CM.SHIP_TO_ID = FILT.SHIP_TO_ID)
		GROUP BY CUSTOMER_ID, SHIP_TO_ID, LEGACY_CUSTOMER
	);
	...
	SELECT	  CM.CUSTOMER_ID
			, CM.SHIP_TO_ID AS CUSTOMER -->>> GENERIC NAME CHANGE CUSTMER TO THE PROPER FIELD
	...
	-- select top 100 * from DB_ADI_NA_QA.SCH_ADI_NA_CORE.FACT_CUSTCREDIT
	--------------------------------------------------------------------------------------
	...
	AS
        SELECT FC.TERMS_KEY, FC.SHIP_TO_ID, FCC.CREDIT_LIMIT AS CRD_LIMIT, FC.AMOUNT_PAID AS CURR_DUE -->>> CHECK IF THIS CORRESPOND TO CURR_DUE ???
        FROM FACT_CREDIT FC
	...
	(
		...
		FROM	T_CM AS CM
				LEFT OUTER JOIN DB_ADI_NA_QA.SCH_ADI_NA_CORE.T_CS AS CS 
					ON CM.CUSTOMER = CS.SHIP_TO_ID -->> CHANGE TO SHIP_TO_ID
				LEFT OUTER JOIN DB_ADI_NA_QA.SCH_ADI_NA_CORE.T_CLP AS CLP
					ON CM.CUSTOMER = CLP.SHIP_TO_ID
				LEFT OUTER JOIN DIM_TERMS AS CT 
					ON CS.TERMS_KEY = CT.TERMS_KEY
			WHERE	EXISTS (SELECT 1 FROM T_CUST AS FILT WHERE CM.CUSTOMER = FILT.SHIP_TO_ID)	
	);
	...
	SELECT	SHIP_TO_ID, 'fy' AS FY,  COALESCE(CAT, 'all') AS CAT, IFNULL(CY, 0) AS CY, IFNULL(PY, 0) AS PY
	FROM	(
				SELECT	T.FY
						, T.CAT, T.SHIP_TO_ID, SUM(T.SLS) AS SLS
				FROM	DB_ADI_NA_QA.SCH_ADI_NA_CORE.T_RAWDATA AS T
				WHERE	FY IS NOT NULL			
				GROUP BY T.FY, T.SHIP_TO_ID, ROLLUP( T.CAT )
			)T PIVOT ( SUM(SLS) FOR FY IN ('CY', 'PY') ) AS P (SHIP_TO_ID, CAT, CY, PY);

    -- Get mtd data:
	INSERT INTO	T_SPEND (CUST, TF, CAT, CY, PY)
    SELECT	SHIP_TO_ID, 'fm' AS FM, COALESCE(CAT, 'all') AS CAT, IFNULL(CYFM, 0) AS CY, IFNULL(PYFM, 0) AS PY
	FROM	(
				SELECT	CAST(T.FM AS varchar) AS FM
						, T.CAT, T.SHIP_TO_ID, SUM(T.SLS) AS SLS
				FROM	DB_ADI_NA_QA.SCH_ADI_NA_CORE.T_RAWDATA AS T
				WHERE	FM IS NOT NULL --AND FM = 'CYFM'
				GROUP BY T.FM, T.SHIP_TO_ID, ROLLUP( T.CAT ) 
			) T PIVOT ( SUM(SLS) FOR FM IN ('CYFM','PYFM') ) as P (SHIP_TO_ID, CAT,CYFM, PYFM);  
	...
	SELECT	SHIP_TO_ID, 'all', MIN(DATE_ID) AS DATE_ID, ORDER_NBR
	FROM	T_RAWDATA
	WHERE	ORDER_TYPE_CODE IN ('NS', 'CC', 'QP')
	GROUP BY SHIP_TO_ID, ORDER_NBR;        
			
		-- Get Orders and collocation date by CAT:
	INSERT INTO	T_ORD0 (CUST, CAT, DATE_ID, ORDER_NBR)
	SELECT	SHIP_TO_ID, CAT, MIN(DATE_ID) AS DATE_ID, ORDER_NBR
	FROM	T_RAWDATA
	WHERE	ORDER_TYPE_CODE IN ('NS', 'CC', 'QP')
	GROUP BY SHIP_TO_ID, CAT, ORDER_NBR	;
	...
	SELECT	SHIP_TO_ID, 'fy', FY, CAT, SUM(SLS) AS SLS
	FROM	T_RAWDATA
	GROUP BY SHIP_TO_ID, FY, CAT;

	-- Get MTD sales by CAT:
	INSERT INTO T_CAT0 (CUST, TF, PER, CAT, SLS)
	SELECT	SHIP_TO_ID, 'fm', LEFT(FM, 2), CAT, SUM(SLS) AS SLS
	FROM	T_RAWDATA
	WHERE	FM IS NOT NULL
	GROUP BY SHIP_TO_ID, LEFT(FM, 2), CAT;
	...
	-- Get prior year sales of YTD
      MERGE INTO T_CAT AS U
        ...
        AND U.TF = 'fy'
        WHEN MATCHED THEN
            UPDATE SET U.PY = COALESCE(S.SLS, 0);
	...
	SELECT	SHIP_TO_ID, 'fy', FY, CAT, RCAT, SUM(SLS)
	FROM	T_RAWDATA T
	GROUP BY SHIP_TO_ID, FY, CAT, RCAT;

	-- Get MTD sales by cat/rcat:
	INSERT INTO T_RCAT0 (CUST, TF, PER, CAT, RCAT, SLS)
	SELECT	SHIP_TO_ID, 'fm', LEFT(FM, 2) , CAT, RCAT, SUM(SLS)
	FROM	T_RAWDATA T
	WHERE	FM IS NOT NULL
	GROUP BY SHIP_TO_ID, LEFT(FM, 2), CAT, RCAT;
	...
	SELECT	SHIP_TO_ID, 'fy', FY, CAT, VENDOR_ID, SUM(SLS), SUM(QTY)
	FROM	T_RAWDATA T
	GROUP BY SHIP_TO_ID, FY, CAT, VENDOR_ID;

	-- Get MTD sales by CAT/Vendor:
	INSERT INTO T_VEND0 (CUST, TF, PER, CAT, VENDOR, SLS, QTY)
	SELECT	SHIP_TO_ID, 'fm', LEFT(FM, 2), CAT, VENDOR_ID, SUM(SLS), SUM(QTY)
	FROM	T_RAWDATA T
	WHERE	FM IS NOT NULL
	GROUP BY SHIP_TO_ID, LEFT(FM, 2), CAT, VENDOR_ID;
	...
	SELECT	SHIP_TO_ID, 'fy', FY, CAT, ITEM_ID, SUM(SLS), SUM(QTY)
	FROM	T_RAWDATA T
	GROUP BY SHIP_TO_ID, FY, CAT, ITEM_ID;

	-- Get MTD sales:
	INSERT INTO T_ITEM0 (CUST, TF, PER, CAT, ITEM, SLS, QTY)
	SELECT	SHIP_TO_ID, 'fm', LEFT(FM, 2), CAT, ITEM_ID, SUM(SLS), SUM(QTY)
	FROM	T_RAWDATA T
	WHERE	FM IS NOT NULL
	GROUP BY SHIP_TO_ID, LEFT(FM, 2), CAT, ITEM_ID;
	...
	WHERE	TRUE 
	        AND FO.QUOTE_FLAG= 'Y' -- need to check this logic
	...
	INSERT INTO T_PR0
	SELECT	CU.SHIP_TO_ID AS CUST, FY, LEFT(FM, 2) FM, MAX(FD.DT) AS DT, LO.CITY, LO.ST, FO.ORDER_NO AS ORDER_NBR, PID.EMP_NAME, QTY_ORDERED AS ORDER_AMT
	FROM	FACT_ORDERS AS FO
	...
	WHERE	FO.QUOTE_FLAG = 'N'
			AND FO.PROJECT_QUOTE_FLAG = 1 -- get quote projects 
	GROUP BY ALL;
	...
	FROM   T_DT DT
				  ...
				  CROSS JOIN (SELECT CAT FROM T_CATDESC UNION SELECT 'all') T
	WHERE  FM = 'CYFM'
				  AND VS.WEEK_DAY BETWEEN 2 AND 6 AND CAT <> '0';
	...
	INSERT INTO TC_MTD0
	SELECT T.SHIP_TO_ID, T.DATE_ID, COALESCE(T.CAT, 'all') CAT, SUM(T.SLS) AS SLS
	FROM   T_RAWDATA AS T
	WHERE  FM IS NOT NULL
	GROUP BY T.DATE_ID, T.SHIP_TO_ID, ROLLUP( T.CAT );
	...
	INSERT INTO TC_YTD0
	SELECT T.SHIP_TO_ID, D.DATE_ID, D.MM, FY, COALESCE(T.CAT, 'all') CAT, SUM(T.SLS) AS SLS
	FROM   T_RAWDATA AS T
				  LEFT OUTER JOIN (SELECT DISTINCT DATE_ID, MM FROM DT_YTD) AS D ON T.MM = D.MM
	GROUP BY D.DATE_ID, D.MM, FY, T.SHIP_TO_ID, ROLLUP( T.CAT );
	...
	CREATE OR REPLACE TEMPORARY TABLE BASE_MTD (
		   ...
		   CAT varchar(3),
		   CUSTOMER_ID VARCHAR(10),
           SHIP_TO_ID varchar(10),
		   LEGACY_CUSTOMER VARCHAR(10)
		   );
	...
	INSERT INTO TC_MTD
	SELECT	DT.SHIP_TO_ID, DT.DATE_ID, DT.CAT, SUM(IFF(DT.DATE_ID = TC.DATE_ID, SLS, 0)) CY, SUM(IFF(DT.DATE_ID_PY = TC.DATE_ID, SLS, 0)) PY
	FROM	BASE_MTD DT
			LEFT OUTER JOIN TC_MTD0 TC 
				ON (DT.DATE_ID = TC.DATE_ID OR DT.DATE_ID_PY = TC.DATE_ID) 
				AND DT.CAT = TC.CAT 
				AND DT.SHIP_TO_ID = TC.CUST

	GROUP BY DT.DATE_ID, DT.CAT, DT.SHIP_TO_ID;
	...
	--
	INSERT INTO TC_YTD
	SELECT DT.SHIP_TO_ID, DT.DATE_ID, DT.CAT, SUM(IFF(TC.FY = 'CY', SLS, 0)) CY, SUM(IFF(TC.FY = 'PY', SLS, 0)) PY
	FROM   BASE_YTD DT
				  LEFT OUTER JOIN TC_YTD0 TC ON DT.MM = TC.MM AND DT.CAT = TC.CAT AND DT.SHIP_TO_ID = TC.CUST
	GROUP BY DT.DATE_ID, DT.CAT, DT.SHIP_TO_ID;
 ...
 -- Create the final OBJECTS
	CREATE OR REPLACE TEMPORARY TABLE T_CUST_DATA AS
		...
		SHIP_TO_ID AS CUSTOMER_NUM,
			(
				...
				FROM CSR_CUST
				WHERE CUST = SHIP_TO_ID
			) AS CUST_DATA,
		FROM T_CUST
	;

	CREATE OR REPLACE TEMPORARY TABLE T_SPEND AS
		SELECT SHIP_TO_ID AS CUSTOMER_NUM, 
		(
	...
	CREATE OR REPLACE TEMPORARY TABLE T_TREND AS
		...
		SELECT SHIP_TO_ID, SHIP_TO_ID AS CUSTOMER_NUM,
			(
...
-- CONSOLIDATE ALL THE TABLES INTO ONE TABLE
	CREATE OR REPLACE TABLE DB_ADI_NA_QA.SCH_ADI_NA_CORE.TBL_CSR_DATA(
		ORIGIN VARCHAR(10),
		P21_CUSTOMER_ID VARCHAR(10),
	    SHIP_TO_ID VARCHAR(10),
	...
	CREATE OR REPLACE TABLE DB_ADI_NA_DEV.SCH_ADI_NA_CORE.TBL_CSR_P21_DATASET AS
	SELECT 'P21' AS ORIGIN, CU.CUSTOMER_ID AS P21_CUSTOMER_ID, T_CUST_DATA.CUSTOMER_NUM AS SHIP_TO_ID, CU.LEGACY_CUSTOMER, CUST_DATA, SPEND, ORDERS, TREND, CAT, RCAT, VENDORS, ITEM_SLS, ITEM_QTY, RETURNS, QUOTES, PROJECTS, TF, CATEGORY
	FROM T_CUST_DATA
		LEFT JOIN T_SPEND 
		...
		LEFT JOIN DIM_CUSTOMER CU 
			ON CU.SHIP_TO_ID = T_CUST_DATA.CUSTOMER_NUM
		
	;

	

	-- INSERT INTO DB_ADI_NA_QA.SCH_ADI_NA_CORE.TBL_CSR_DATA
	-- 	(ORIGIN, P21_CUSTOMER_ID, SHIP_TO_ID, CUSTOMER_NUM, CUST_DATA, SPEND, ORDERS, TREND, CAT, RCAT, VENDORS, ITEM_SLS, ITEM_QTY, RETURNS, QUOTES, PROJECTS, TF, CATEGORY)
	-- SELECT * FROM 
		
		select * from DIM_CUSTOMER where SHIP_TO_ID = 'U4803000';
	  SELECT * FROM T_CUST WHERE SHIP_TO_ID = 'U4803000';
		
	-- SELECT * FROM DB_ADI_NA_QA.SCH_ADI_NA_CORE.TBL_CSR_DATA;
	...
	ALTER TABLE DB_ADI_NA_QA.SCH_ADI_NA_CORE.TBL_CSR_DATA
	RENAME COLUMN CUSTOMER_NUM TO LEGACY_CUSTOMER;
------------------------------------------------------------------------------------------------
-- APPEND DATA

INSERT INTO DB_ADI_NA_QA.SCH_ADI_NA_CORE.TBL_CSR_DATA
SELECT * FROM DB_ADI_NA_DEV.SCH_ADI_NA_CORE.TBL_CSR_P21_DATASET
UNION ALL
SELECT * FROM DB_ADI_NA_DEV.SCH_ADI_NA_CORE.TBL_CSR_HARMONIZED_DATASET;
